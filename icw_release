...
if ($ENV{'ICW_REPO'} eq "") { 
    printf("Error: Environment variable ICW_REPO not defined\n\n");
    printf("export ICW_REPO=repo_name\n");
    exit(1);
}

my $repo = $ENV{'ICW_REPO'};
...
{ printf("icw commit is not allowed\nUse svn insted\n");
  ...
  { $cmd = sprintf("$svn ci $tag -m '$commit_note'");
    ...
    $local_comp_name  = $ARGV[1];   # library name
    $comp_name = $ARGV[1];
    ...
    printf("Debug: $local_comp_name $comp_name\n");
    ...
    $repo_target      = $ARGV[2];   # check must be <analog|digital|process|tools>[/category]/
